...
abstract class DataBaseCore extends Licencing
{
    ...
    private $build_query_options = array(
                                        'use_SQL_CALC_FOUND_ROWS' => 0,
                                        'use_order_random' => 0
                                        );
    ...
    final protected function ArrayIntersectKey(array &$array)
    {
        $array_new = $array;
    ...
    final public function SetDirectWhere($where, $operand = 'AND')
    {
        if ($operand && !in_array(strtoupper($operand), array("AND", "OR", "XOR")))
        {
            $this->Bug("Unknow operand for set where");
        ...
        if ($this->cur_where)
        {
            $this->cur_where .= " " . $operand . " " . $where;
        }
        else
        {
            $this->cur_where = $where;
        }
    }
    ...
    final public function SetBeginBlockWhere()
    {
        ...
        if (!isset($this->block_where[$this->count_block_where]))
        {
            $this->block_where[$this->count_block_where] = array();
    ...
    final public function GetColumns($table)
    {
        ...
        {
            throw new ExceptionDataBase("Table $table doesn't exist");
        }
    }
    ...
    final public function ExecuteBuildQuery($order = array(), $count = array(), $options = array())
    {
        $this->build_query_options = array_merge($this->build_query_options, $options);
        
        $param = $this->GetParametrForBuildQuery();
        
        if ($this->build_query_options['use_SQL_CALC_FOUND_ROWS'])
        {
            $param['selection'] = "SQL_CALC_FOUND_ROWS " . $param['selection'];
        }
        
        if ($this->build_query_options['use_order_random'])
        {
            $order = " ORDER BY RAND()";
        }
        else 
        ...
        $this->query .= $this->GetWhere(array()) . $this->group . $order . $this->GetLimit($count);
                    ...
                    elseif (is_array($action) && $action)
                    {
                        foreach ($action as $table=>$value)
                        {
                            foreach ($value as $column=>$as)
                            {
                                ...
                                {
                                    if (!preg_match('#COUNT|SUM#', $column))
                                    {
                                        if ($as && !is_numeric($as))
                                        {
                                    ...
                                    {
                                        $selection .=  $column . " as " . preg_replace('#[^a-z]#i', "_", $as);
                                    }
                                }
                            }
                        }
    ...
    abstract public function FetchArray($resource = null, $result_type = MYSQL_ASSOC);
