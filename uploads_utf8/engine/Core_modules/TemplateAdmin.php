...
final class TemplateAdmin extends Licencing
{
     ...
     * @param array $script
     */
    public function header($header_text = '', $menu = false, $subhead = '', array $css = array(), array $script = array())
    {
        if ($this->head)
        {
            ...
            foreach ($css as $cs)
            {
                echo '<style type="text/css" media="all">@import url(' . $this->base_path . trim($cs, "/") . ");</style>\n";
            }
            echo $this->script;
            foreach ($script as $sc)
            {
                echo '<script type="text/javascript" src="' . $this->base_path . trim($sc, "/") . "\"></script>\n";
            }
            echo $this->subhead;
            echo $subhead;
        }
    ...
    public function SetStyleScript($style = array(), $script = array())
    {
        ...
        {
            $this->style .= '<style type="text/css" media="all">@import url(' . $this->base_path . trim($css, "/") . ");</style>\n";
        }
        foreach ($script as $sc)
        {
            $this->script .= '<script type="text/javascript" src="' . $this->base_path . trim($sc, "/") . "\"></script>\n";
        }
    }
...
HTML;
    ...
    public function footer($footer = false, $year = '')
    {
        ...
        {
            echo "<pre>";
            echo wordwrap(print_r(self::$Debug_info, true));
            echo "</pre>";
        }

        $cur_yaer = date('Y');

        if ($year && $year < $cur_yaer)
        {
            $year = $year . " - ";
        }
        else if ($this->year && $this->year < $cur_yaer)
        {
            $year = $this->year . " - ";
        }
        else if ($cur_yaer == $year)
        {
            $year = '';
        }
            ...
            foreach ($hidden as $key=>$value)
            {
                if (is_array($value))
                {
                    foreach ($value as $key2=>$value2)
                        $output .= "<input type=\"hidden\" name=\"{$key}[{$key2}]\" value=\"$value2\" />\n";
                }
                else 
...
HTML;
     ...
     * @param string $script
     * @param boolean $line
     * @return string|TemplateAdmin
     */
    public function OTable(array $thead = array(), $script = "", $line = true)
    {
        $this->CountTable++;
