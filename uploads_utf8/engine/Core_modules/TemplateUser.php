...
final class TemplateUser extends Licencing
{
    ...
    public  $current_url = array();
    
    public $base_path = '';

    private $title;
    private $speedbar;
    public  $separator_title = " &raquo; ";
    public  $separator_speedbar = " &raquo; ";

    private static $style = '';
    private static $script = '';
    public static $show_sub = false;
    public static $show_superhead = false;
    ...
    /**
     * Open new row with name $row_name
     * 
    ...
    public function SetRow($replace, $row_name)
    {
        ...
        {
            if ($this->row_count[$row_name] && $this->row_count[$row_name]%$this->sep_count[$row_name] == 0)
            {
    ...
    public function SetStyleScript($style = array(), $script = array())
    {
        foreach ($style as $css)
        {
            ...
            {
                self::$style[] = $css;
                if (strpos($css, "{THEME}") !== false)
                {
                    $this->style_cur .= '<style type="text/css" media="all">@import url(' . $css . ");</style>\n";
                }
                else
                {
                    $this->style_cur .= '<style type="text/css" media="all">@import url(' . $this->base_path . trim($css, "/") . ");</style>\n";
                }
            }
        }
        foreach ($script as $sc)
        {
            if (empty(self::$script[$sc]))
            {
                self::$script[] = $sc;
                $this->script_cur .= '<script type="text/javascript" src="' . $this->base_path . trim($sc, "/") . "\"></script>\n";
            }
        }
        return $this;
    }
    ...
    public function WrapContent($begin = '', $end = '')
    {
        ...
        $ext_content = $begin . $ext_content . $end;
    ...
    {
        return "<input type='radio' name='$name' value='$value' $script />";
    }

    public function TextArea($name, $value = '', $script = '')
    {
        return "<textarea name='$name' $script >$value</textarea>";
    }
    ...
    private function GetAltUrl(&$url_array, $optinos)
    {
        ...
        return $url_str . $optinos['alt_ext'];
    }
    ...
    private function &CurrentURL()
    {
        ...
        preg_match_all('#([^&=\?]+)=([^&]*)#i', $this->main_url, $matches_main);
        
        preg_match_all('#([^&=]+)=([^&]*)#i', $_SERVER['QUERY_STRING'], $matches);
        
        $matches[1] = array_diff_key($matches[1], $matches_main[1]);
        
        foreach ($matches[1] as $index => $var)
        {
            $this->current_url[$var] = $matches[2][$index];
        }
        
        return $this->current_url;
    } 

    public function TitleSpeedBar($text = '', $link  = '', $options = array())
    {
        $options_default = array(
                                'sep_title' => $this->separator_title,
                                'sep_speedbar' => $this->separator_speedbar
                                );
        ...
        if ($text)
        {
            ...
            if ($this->speedbar)
            {
                $this->speedbar .= $options['sep_speedbar'];
            }
    ...
    public function PageNavigation(array $url, $count_all, $per_page, $cur_page = 1, $i = 0, array $options = array())
    {
        ...
        if (isset($cstart) and $cstart != "" and $cstart > 0)
        {
            ...
            if ($prev > 1)
            $page_url = $this_2->GetUrl($url, array(), array("page" => $prev), array("page"), $options);
            else
            $page_url = $this_2->GetUrl($url, array(), array(), array("page"), $options);
        ...
        if($per_page)
        {
            if($count_all > $per_page)
            {
                ...
                if ($enpages_count <= 10 )
                {
                    ...
                    {
                        if ($j > 1)
                        $page_url = $this_2->GetUrl($url, array(), array("page" => $j), array("page"), $options);
                        else
                        $page_url = $this_2->GetUrl($url, array(), array(), array("page"), $options);
                ...
                else
                {
                    ...
                    if ($start >= 2)
                    $pages .= "<a {$options['link_script']} href=\"" . $this_2->GetUrl($url, array(), array(), array("page"), $options) . "\">1</a> ... ";
                    ...
                    $pages .= $nav_prefix."<a {$options['link_script']} href=\"" . $this_2->GetUrl($url, array(), array("page" => $enpages_count), array("page"), $options) . "\">{$enpages_count}</a>";
                    else
        ...
        if($per_page AND $per_page < $count_all AND $i < $count_all)
        {
            ...
            $this_2->Set("<a {$options['link_script']} href=\"" . $this_2->GetUrl($url, array(), array("page" => $next_page), array("page"), $options) . "\">", "[next-link]");
    ...
    {
        throw new ExceptionAllError("Call undefined method " . $me);
    }
