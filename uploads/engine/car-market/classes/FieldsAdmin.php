...
class FieldsAdmin
{
    ...
    public function AddField($type, array $data)
    {
        ...
        if (!$this->_errors)
        {
            ...
            return $this->_db->Insert('auto_fields', array(
                                                    'title'       => $data['title'],
    ...
    protected function _check_error(array $data)
    {
        ...
        if (empty($data['title']))
        {
            $this->_errors[] = $this->_auto->lang['xfields_error_empty_title'];
        }
    }
    ...
    protected function _pre_save($type, array &$data)
    {
        ...
        switch ($type) 
        {
            case 'select':
                if (empty($data['data']))
                {print_r($data);
                    $this->_errors[] = $this->_auto->lang['xfields_error_select_data'];
                }
                else 
                {
                    $opts = explode("\n", $data['data']);
    ...
    public function UpdateField($id, array $data)
    {
        ...
        if (!$this->_errors)
        {
            ...
            $this->_db->Update('auto_fields', array(
                                                    'title'       => $data['title'],
                                                    'description' => $data['description'],
                                                    'data'        => $field_data,
                                                    'required'    => $data['required'],
    ...
    {
        $field = $this->_db->SelectOne('auto_fields', array('*'), array('id' => $id));
    ...
    public function GetFields()
    {
        ...
        $this->_db->Select('auto_fields', array('title', 'required', 'type', 'default', 'id', 'active'));
